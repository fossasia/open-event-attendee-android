...
object JWTUtils {
    ...
    fun getExpiry(token: String): Long =
        java.lang.Long.parseLong(JSONObject(decode(token).get(1)).get("exp").toString())

    @Throws(JSONException::class)
    fun getIdentity(token: String): Long =
        java.lang.Long.parseLong(JSONObject(decode(token).get(1)).get("identity").toString())
    ...
    private fun getJson(strEncoded: String): String =
        String(Base64Utils.decode(strEncoded))
